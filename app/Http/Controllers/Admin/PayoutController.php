...
class PayoutController extends Controller
{
    ...
    $this->middleware('permission:PayOut', ['only' => ['index','store','create','edit','destroy','update']]);
    ...
    public function getData(Request $request)
    {
        ...
        $records = Transaction::fetchPayOutData($request, $this->columns);
        $total = $records->get();
...
protected function exportPDF($records)
{
    ...
    $pdf = Pdf::loadView('exports.payout_pdf', $data)
              ->setPaper('a4', 'landscape'); // or portrait

    return $pdf->download("payOut_" . now()->format('Ymd_His') . ".pdf");
}

protected function exportCSV($records)
{
    $filename = "payOut_" . now()->format('Ymd_His') . ".csv";
    $headers = [
